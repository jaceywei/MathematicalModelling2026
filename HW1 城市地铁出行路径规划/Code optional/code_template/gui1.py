...
from metro_algorithm1 import Graph, MetroSystem, detect_cities
...
class MetroApp:
    ...
    def _on_solve(self):
        ...
        # 在此处接入换乘功能
        # 1. 接收三个返回值（多了一个 logic_path）
        cost, path, logic_path = self.metro.shortest_path(src_name, dst_name)
        ...
        # 2. 解析 logic_path，生成人性化的换乘提示
        route_output = []
        current_line = None
        segment_stations = []

        for logic_id in logic_path:
            # 从换乘图中提取这个逻辑节点的详细信息
            attrs = self.metro.trans_graph.nodes[logic_id]
            phys_name = self.metro.stations[attrs['original_id']]
            line = attrs['line']

            if current_line is None:
                # 刚进站，记录第一条线
                current_line = line
                segment_stations.append(phys_name)
            elif line == current_line:
                # 还在同一条线上，追加站名（避免同站重复添加）
                if segment_stations[-1] != phys_name:
                    segment_stations.append(phys_name)
            else:
                if len(route_output) == 0:
                    route_output.append(f"  Take {current_line}:\n    " + " -> ".join(segment_stations))
                else:
                    route_output.append(f"  Interchange to {current_line}:\n    " + " -> ".join(segment_stations))
                
                # 开始记录新线路的行程（换乘站作为新线路的起点）
                current_line = line
                segment_stations = [phys_name]
        ...
        self._log(
            ...
            "  Route  :\n{route}\n"
            "{line}".format(
                line="-" * 50, src=src_name, dst=dst_name,
                time=(cost/35)*60, stops=len(path), route=final_route_str,
            )
        )
    ...
    def _draw_path(self, path: list[int], cost: float):
        ...
        self.ax.set_title(
            "{city} Metro Network\n"
            "Shortest Path: {src} -> {dst}  (time = {time:.1f} min)".format(
                city=self.metro.city,
                src=stations[path[0]], dst=stations[path[-1]], time=(cost*60)/35,
